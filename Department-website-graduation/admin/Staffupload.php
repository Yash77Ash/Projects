  ...
  <body>
    ...
    <div class="main-content">
      <header>
        ...
        <form action="Staffupload2.php" method="post" enctype="multipart/form-data">
		<label for="teacher_photo">Teacher Photo:</label>
		<input type="file" id="teacher_photo" name="teacher_photo"><br><br>
		
		<label for="teacher_name">Teacher Name:</label>
		<input type="text" id="teacher_name" name="teacher_name"><br><br>
		
		<label for="teacher_subject">Teacher Subject:</label>
		<input type="text" id="teacher_subject" name="teacher_subject"><br><br>
		
		<label for="teacher_file">Teacher File:</label>
		<input type="file" id="teacher_file" name="teacher_file"><br><br>
		
		<input type="submit" value="Submit">
	</form>
  <hr><br>
  <h2>Uploaded Teachers:</h2><br>
...
<?php
		...
		if (isset($_GET["delete_id"])) {
			...
			$sql = "DELETE FROM teachersinfo WHERE id=$delete_id";
			if ($conn->query($sql) === TRUE) {
				echo "<p>Teacher record deleted successfully.</p>";
			} else {
			...
			<?php
				if ($result->num_rows > 0) {
					...
					while($row = $result->fetch_assoc()) {
						...
						echo "<td><a href='" . $row["teacher_file"] . "' target='_blank'>View File</a></td>";
						echo "<td><button class='delete-btn' onclick='deleteTeacher(" . $row["id"] . ")'>Delete</button></td>";
						echo "</tr>";
					}
				} else {					echo "<tr><td colspan='5'>No teachers found.</td></tr>";
				}
			?>
		</tbody>
	</table>
	...
	<script>
		function deleteTeacher(id) {
			if (confirm("Are you sure you want to delete this teacher?")) {
				window.location.href = "Staffupload.php?delete_id=" + id;
			}
		}
	</script>
