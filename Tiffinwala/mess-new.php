    ...
    if(isset($_SESSION['id']))
{
	...
	if(isset($register))
     {
	$sql=mysqli_query($con,"select * from mess where mess_email='$email'");
	...
	$sql=mysqli_query($con,"insert into mess 
	(mess_name	,mess_email,mess_password,mess_mob,mess_address,mess_logo)
       	values('$r_name','$email','$pswd','$mob','$address','$logo')");
	
    if($sql)
	{
	mkdir("img/Mess");
	mkdir("img/Mess/$email");
	
	move_uploaded_file($_FILES['logo']['tmp_name'],"img/Mess/$email/".$_FILES['logo']['name']);
	}
	$_SESSION['id']=$email;
	       ...
	       <div class="tab-pane fade show active" id="register" role="tabpanel" aria-labelledby="home-tab">
			    ...
			    <form action="" method="post" enctype="multipart/form-data">
                      <div class="form-group">
                          ...
                          <input type="text" class="form-control" id="name" value="<?php if(isset($r_name)) { echo $r_name;}?>" placeholder="Enter Mess Name" name="r_name" required/>
                      </div>
	                  <div class="form-group">
