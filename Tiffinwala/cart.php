...
if(isset($del))
{
	...
	{
		header("location:Cart_delete.php");
	}
 ...
 //update section
  ...
  $fld_name=$det_res['cust_name'];
  $fld_email=$det_res['cust_email'];
  $fld_mobile=$det_res['cust_mobile'];
  $fld_password=$det_res['cust_password'];
  if(isset($update))
  {
	   
	 if(mysqli_query($con,"update customer set cust_name='$name',cust_mobile='$mobile',cust_password='$pswd' where cust_email='$fld_email'"))
      {
                 ...
                 <table class="table">
                  <tbody>
	               <?php
					  ...
					  if ($re) {
						  while ($res = mysqli_fetch_array($query)) {
							  $vendor_id = $res['mess_id'];
							  // Use $vendor_id instead of $mess_id
							  $v_query = mysqli_query($con, "SELECT * FROM mess WHERE mess_id='$vendor_id'");
							  $v_row = mysqli_fetch_array($v_query);
							  $em = $v_row['mess_email'];
							  $nm = $v_row['mess_name'];
	                ?>
                      <tr>
                         <td><image src="img/Mess/<?php echo $em."/foodimages/".$res['image'];?>" height="80px" width="100px"></td>
                   ...
                   <?php
					  ...
					  <a href="Payment.php?total=<?php echo end($gtotal); ?>"><button type="button" style="color:white; font-weight:bold; text-transform:uppercase;" class="btn btn-warning">Proceed to checkout</button></a>
...
<!--tab 1 ends-->	   
            ...
            <div class="tab-pane fade" id="orders" role="tabpanel" aria-labelledby="orders-tab">
				...
				<th>Cancel order</th>
				    <tbody>
					<?php
				$quer_res = mysqli_query($con, "SELECT * FROM `order` WHERE `email_id`='$cust_id' AND `status`='In Process'");
				...
				if (!$quer_res) {
					...
					while ($roww = mysqli_fetch_array($quer_res)) {
						...
						// Note: You might want to perform further processing here
